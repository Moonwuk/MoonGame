  ...
  if (selFleet) {
    ...
    if (f) {
      ...
      if (f.owner === ME && fleetHasSquadron(f)) {
        ...
        h += `<div class="row">`;
        h += btn('qscramble', '', scrambling ? t('🛩 дежурный вылет: ВКЛ') : t('🛩 дежурный вылет: выкл'), true);
        h += `</div>`;
        if (scrambling && pt) {
      ...
      if (f.owner === ME) {
        ...
        h += `<div class="sec">${t('Очередь приказов')}</div><div class="row">`;
        ...
        h += `<div class="hint">${t('Включите «строить», тапайте миры (переходы) и добавляйте «штурм» / «погрузку» / «ждать N ч». «Погрузка» забирает гарнизон захваченного мира обратно в трюм, «ждать» — отложенный приказ (выждать момент). Так одна армия проходит цепочку (переход→штурм→погрузка→ждать→…) сама, пока вы вышли.')}</div>`;
