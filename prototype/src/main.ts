...
function buildingDossier(id: string, level: number): Dossier | null {
  ...
  switch (id) {
    ...
    case 'farm':
      return {
        name,
        body: t('Ярусы гидропонных оранжерей под спектральными лампами позволяют вашим подопечным питаться, ведь голод беспощаден. Выращивает {f} пищи в час. Ваши рабочие и воины едят каждый день, было бы глупо проиграть сражение из-за голодного обморока.', { f: hl(lv.produces.food ?? 0) }),
      };
    case 'power_plant':
      return {
        name,
        body: t('Термоядерный реактор питает энергией ваши миры, он производит {e} энергии в час. Энергия — кровь ваших построек, ведь они работают не на волшебстве. При дефиците всё проседает до половины мощности.', { e: hl(lv.produces.energy ?? 0) }),
      };
    case 'fabricator':
      return {
        name,
        body: t('Чистые цеха литографии печатают {m} микроэлектроники в час. Прожорлива к энергии и людям, зато её продукция ведёт эскадрильи и открывает осадные доктрины. Апгрейды окупаются собственной продукцией.', { m: hl(lv.produces.microelectronics ?? 0) }),
      };
    default:
