...
export function createMatchLoader(deps: MatchLoaderDeps): (matchId: string) => Promise<LoadedMatch | null> {
  ...
  return async (matchId: string): Promise<LoadedMatch | null> => {
    ...
    // The committed path already persists each action; `observe` only re-arms the
    // driver, as an action may have scheduled a new event the sleeping timer can't see.
