...
import { isVisibleTo, visibleState } from './visibility';
...
describe('visibleState (fog of war as a security boundary)', () => {
  ...
  it('keeps stances public but shows only offers the viewer is party to', () => {
    const state = scenario();
    state.players.p3 = player('p3');
    state.diplomacy = { 'p1|p2': 'peace', 'p2|p3': 'alliance' }; // public knowledge
    state.diplomacyOffers = {
      'p1|p2': { from: 'p2', stance: 'pact' }, // p1 is a party → visible
      'p2|p3': { from: 'p3', stance: 'alliance' }, // third-party courtship → hidden
    };
    const view = visibleState(state, 'p1', data);
