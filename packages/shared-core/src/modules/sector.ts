 ...
 * `sectorType` (data-driven, `data/sectors.json`); this module turns those types
...
export const sectorModule: GameModule = {
  ...
  setup(api) {
    api.hook<number>('fleet.speed', (speed, args, h) => {
      ...
      const type = to ? h.state.planets[to]?.sectorType : undefined;
    ...
    api.hook<number>('combat.damage', (dmg, args, h) => {
      ...
      const type = planet.sectorType ? h.ctx.data.sectors[planet.sectorType] : undefined;
