...
Мобильная пошагово-асинхронная космическая grand strategy с массовым мультиплеером —
в духе игр Bytro Labs, но в сеттинге тёмного космоса. Жанр и темп — как вдохновение;
весь код, контент и механики оригинальные.
...
| `kernel/` | Микроядро: `createKernel`/`applyAction`, шина событий, хуки, реестр возможностей, манифест |
...
- **Этап 1 (Ядро)** — 🚧 заложен фундамент (kernel + шина + реестр модулей + `GameState` +
  seeded RNG + `applyAction`). Дальше — `advanceTo`, базовые модули, движок трейтов.
