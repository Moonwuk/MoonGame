...
| **1 · Ядро** (`shared-core`) | ✅ Готово | 22 модуля (ниже), микроядро + `advanceTo`, детерминизм (sfc32 + golden), fail-secure | **EFX-1** — универсальный движок трейтов/эффектов (backlog; трейты читаются точечно) |
...
- CI `security.yml`: сканеры (Semgrep/CodeQL/Trivy/OSV/Gitleaks/TruffleHog/zizmor + SBOM) —
  **информационный/не-блокирующий** (ratcheting — SEC-1 в бэклоге).
...
2. **P0-баги сетевого матча:** порог победы в сети = 600 при HUD «450» (config не передаётся
   в MatchRoom); место p2 создаётся без совета учёных (ai-флаг снимается после `newGame`);
   доменные события (ПВО/артиллерия/потери) сервер шлёт, но клиент их отбрасывает —
   боевые эффекты не видны по сети; сетевого чата нет; смягчение стойки между людьми
   заблокировано заглушкой.
...
6. **EFX-1** — движок эффектов в ядре (трейты читаются точечно).
7. **Мультипроцессный масштаб сервера.**
8. **Метаигра / AvA-сектора** — отложено надолго.
