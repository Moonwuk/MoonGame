...
Статус после F8: пункты 1–2 **сделаны** в обоих серверах (прото-сервер имел их с
PA-4.1; `main.ts` получил паритет в F8). Пункт 3 — открытый риск. Не требуют Fastify
(SV-0.1) — швы уже были готовы.
...
3. **⚠️ Overflow-клин (открытый риск).** Гварды `MAX_EVENTS_PER_STEP=10_000` /
