...
- **SHIP-7** 🔒(SHIP-1) `[proto][core]` Носитель + десантная авто-высадка. Корпус-носитель
  (большой трюм, мало пушек) **готов** (`dropship` юнит, `cargoCapacity 8` / `attack 2`).
  Дальше: трейт `landing`; дивизия в трюме **сама высаживается** на враждебный/ничейный
  захватываемый сектор, когда орбита чиста (нет вражеского флота); на Void-станцию
  (`orbit:false`) — сразу. **HP высаженной дивизии ∝ доле HP носителя** при высадке
  (битый носитель — слабее десант). Переиспользует `runGroundCombat`/`captureGround`. Тесты.
