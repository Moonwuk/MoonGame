...
- **SHIP-2** 🔒(SHIP-1) `[proto]` UI «Верфь»: вкладка в setup (как «Дивизии»/«Герои») —
  фиттинг модулей в слоты корпуса, **переиспользуя инвентарь-UI героев**; превью
  деривированных статов; лоадауты в `SetupConfig` (заморожены на старте).
