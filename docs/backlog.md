...
- **EFX-1** ⏳ `[core]` **Универсальный движок трейтов/эффектов.** Интерпретатор
  `EffectRuleSchema`/`data/events.json` (сейчас только валидируются): модуль-исполнитель
  правил поверх шины/хуков; юнитовые `traits` читаются генерически, а не точечно
  (artillery/immobile/hero). Разгребает главный дизайн-долг Этапа 1.
