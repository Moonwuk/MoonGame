...
> EC-1..3) — это чистая внутриматчевая модель, как герои до сервера; **(г)** сквадроны —
> **палубная авиация**: сидят в трюме носителя и **вылетают** на цель (короткая дальность,
...
- **SHIP-7** 🔒(SHIP-1) `[proto][core]` Десантная эскадрилья = **конверсия в дивизию**
  (НЕ груз в трюме). **Свой юнит** `landing_squadron`: при старте постройки выбираешь
  **целевую дивизию**; стоит дороже обычной дивизии (+~25% — за доставку/конверсию);
  `cargoSize 2` (2 слота трюма). **Вылетает из трюма носителя** (палубная авиация,
  короткая дальность) — авто, когда носитель в радиусе цели; при штурме враждебного/
  ничейного **захватываемого** сектора (орбита чиста — нет вражеского флота; Void-станция
  `orbit:false` — сразу) **конвертируется в выбранную дивизию**, та воюет на земле (reuse
  `runGroundCombat`/`captureGround`). **HP дивизии ∝ доле HP эскадрильи** на момент
  конверсии (сбили на подлёте → слабее). Зависит от SHIP-11 (выпуск). Тесты.
  _(Носитель — отдельный готовый юнит `dropship`: `cargoCapacity 8` / `attack 2`.)_
- **SHIP-8** 🔒(SHIP-11) `[data][core]` Эскадрильи — малые корабли (истребитель/бомбер…),
  тип/трейт `squadron`, разные роли; **палубные** — едут в трюме носителя (полиморфный груз:
  дивизия | эскадрилья) и вылетают на цель (короткая дальность). Десант-юнит — тоже
  `squadron` (цель для ПВО, см. SHIP-10). Тесты.
- **SHIP-9** 🔒(SHIP-7) `[core]` Десант **бьётся с гарнизоном**: снять «шов» в `captureGround`
  — гарнизон (`marine`/`orbital_aa`) защищает мир в наземном бою, захват только после
  зачистки. _(Развилка: гарнизон-ростер ≠ дивизион-ростер `GROUND_ROSTER` — нужен мост/
  единая модель.)_ Тесты.
- **SHIP-10** 🔒(SHIP-8) `[core]` Ближнее ПВО против эскадрилий — по образцу `runArtillery`
  (`combat.ts`): малый радиус, **залп каждые 15 мин**, цель — юниты типа `squadron`, **урон
  делится по всем эскадрильям в радиусе** (AoE, не одиночная цель как артиллерия). Связка:
  ПВО сбивает входящий носитель-десант → (через SHIP-7) дивизия высаживается ослабленной. Тесты.
- **SHIP-11** 🔒(SHIP-1) `[core]` **Выпуск/возврат палубных сквадронов** (фундамент для
  SHIP-7/8/10). Сквадрон вылетает из трюма носителя как короткодальний борт у цели
  (опираемся на существующий `fleetLaunch`/rally-концепт прототипа); боевые возвращаются на
  носитель (перезарядка / радиус действия от носителя), десант — в один конец (конверсия).
  Авто-выпуск, когда носитель в радиусе цели. Детерминизм/JSON-стейт. Тесты.
