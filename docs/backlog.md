...
> каркас **action-layer**, multiplayer-slice, **F6 туман на рассылке**, играбельный прототип. 254 теста зелёные.
...
- **A2** 🚧 Радарная дальность от зданий: **радар-постройка с 3 уровнями** в `data/buildings.json`
  (`radarRange` 2→3→4), `radarRange` стал **уровневым** (`BuildingLevelSchema`), `visibleState`
  читает через `buildingLevel`. Осталось: хук `vision.source`/`radar.source` (дать тех/фракциям
  вклад в дальность) + сенсорная дальность зданий (не только радар).
