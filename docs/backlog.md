...
- **ST-3.3** ✅ `[core]` `[srv]` `[proto]` **Поза «Активная оборона».**
  ...
  по припаркованному врагу своего узла (сильнейшее крыло, прогноз удара — победа с
  потерями < порога; один атакующий на цель) + fire-watch `order.scramble` на
  эскадрилье-крыльях у своих миров (снимается при эвакуации). Территорию не покидает,
  войн не объявляет. UI выбора позы + RU/EN. +5 тестов.
