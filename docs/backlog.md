...
- **D4** ◐ `[proto]` Миграция прототипа на ядровый `diplomacyModule`. **Игровая часть
  закрыта NETP0-5**: прототипный `diplomacy.declare` теперь несёт те же D2/D3-семантики
  на хелперах ядра (consent-офферы через `getOffer`/`setOffer`/`clearOffers`, эскалация
  стирает переговоры, `E_ALREADY`/`E_ALREADY_OFFERED`), UI умеет офферы (✓/⏳), бот
  отвечает по favour-шкале (принимает ≥ порога, иначе отклоняет со стиранием оффера).
  Остаток — механическое дедуплицирование: убрать прототипный модуль из `MODULES` и
  подключить ядровый (семантики уже совпадают; мешает только прототипный посев `peace`
  и `E_BAD_TARGET`-нюансы). Низкий приоритет — поведение уже единое.
...
- **HERO-4** ⏳ _(HERO-1 ✅ — замок снят)_ Обобщённый `hero.ability {heroId, abilityId, target}` + диспетчер по
