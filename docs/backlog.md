...
- **ST-3.2** ✅ `[srv]` `[proto]` **Эвакуация под угрозой (поза `defend`).**
  ...
  прогноз → ≥ порога → доккнутые флоты грузят гарнизон в свободные трюмы и уходят в
  ближайший безопасный свой узел; застрявшему гарнизону — ближайший idle-паром, только
  если успевает с запасом 2ч до ETA (`E_UNDER_ASSAULT` блокирует погрузку после начала
  штурма — опоздавший не посылается). Один флот — одна задача; дубль-паром не шлётся;
  негде укрыться → hold. 7 тестов (`stewardGuard.test.ts`).
- **ST-3.3** ⏳ `[core]` `[srv]` `[proto]` 🔒(ST-3.2) **Поза «Активная оборона».**
  Новая поза: < порога → контрудар по видимой цели, только на своих узлах, только
  стойка войны; нейтралов не трогать. UI выбора позы при делегировании.
