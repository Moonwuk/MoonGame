...
- **AVA-9** ✅ `[srv]` **Публичная лента корпораций.** `AvaFeedStore` (append-only,
  ...
  `AvaService.accept`/`settleMatch` (последним шагом, best-effort — не валит транзакцию);
...
- **ONB-1** ✅ `[proto]` ★ **Движок гайд-марок (spotlight).** Переиспользуемый примитив:
  чистый движок `spotlight.ts` (шаги из данных, `parseAdvance`/`parseTour` fail-secure,
  продвижение по `tap`/`action:<type>`/`state:<pred>`, пропуск, optional-скип/безопасный
  стоп на отсутствующем target; иммутабельно, как `onboarding.ts`) + тонкий DOM-контроллер
  `spotlightDom.ts` (оверлей+подсветка bbox+пузырь+счётчик «Шаг k из n»+«Пропустить»/«Далее»,
  re-query цели на каждом `poll` — устойчив к перерисовке; **инжектируемый `document`** →
  тестируется без браузера) + CSS в `build.mjs` (слой 44: над HUD, под codex-модалкой) +
  i18n RU/EN. Тесты: движок (tap/action/state/skip/optional-missing) + контроллер на
  фейковом DOM (рендер/кнопки/предикат/teardown). На нём строятся ONB-2/3 (конкретный тур +
  хуки `notifyAction`/`poll` в игровой цикл — их дело).
