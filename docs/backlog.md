...
- **LARS-1** ✅ `[core/srv]` **Живая авторизация постройки.** Гейт владения перенесён в
  живую проверку: `MatchRoom.authorizePlayerAction` (async wire-правило до допуска, без
  квитанции, серверные драйверы мимо, бросок → `E_UNAVAILABLE`) + `liveArsenalGate` —
  `unit.build`/`hero.fit` сверяются с `ArsenalStore.listOf` в момент сабмита
  (`E_NOT_OWNED`; купленное в матче строимо сразу). AvA-обвязка через `matchExtras`
  (slot→account из `AvaSession.seats`); оркестратор больше не сеет снапшот — живое
  владение единственный источник (LARS-0.2). Детерминизм цел: ядро видит только
  допущенное действие, реплеи не перепроверяют владение.
