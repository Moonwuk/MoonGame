...
- **H5** ✅ Шпионаж играбелен (SPY-1 → прототип): ядровый `espionageModule` в `MODULES`
  ...
  4 e2e-теста. Дальше: **SPY-2** контрразведка (шанс обнаружения жертвой, цена, хук
  `espionage.chance` со стороны защиты), **SPY-3** перехват пингов/приказов (metagame
  §шпионаж) — ядровые кирпичи, ещё не нарезаны.
