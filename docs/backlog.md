...
- **EFX-1** ✅ `[core]` **Универсальный движок трейтов/эффектов.** `effectsModule`
  (`modules/effects.ts`) исполняет правила `data/events.json` по модели architecture.md
  («Три уровня гибкости», ур. 2): ключ правила = id трейта — носитель трейта исполняет
  одноимённое правило по триггеру, generic, без кода на трейт. Курируемые каталоги
  (паттерн «enum, не открытая строка»): триггеры `planet_captured` (носитель — юнит
  захватчика на узле: флот или севший десант) и `schedule` (тики по фиксированной сетке
  `startedAt + k·cadence`, исполняет спан `time.advanced`, который их накрыл, —
  результат не зависит от гранулярности advanceTo (композитность/реплей-тождество,
  подтверждено тестом one-jump ≡ инкрементально вкл. RNG); пол каденса 1ч fail-secure);
  эффекты `add_trait` (дедуп) и `modify_resource` (казна владельца, клэмп на 0).
  `chance` детерминирован (сорт правил/носителей; 0/1 не тратят RNG-поток). Неизвестный
  триггер/эффект/params → правило инертно (graceful). Подключён в оба хоста (прототип —
  `events:{}` → инертен; носителей в `data/units.json` нет → баланс не изменён). Движковые
  поведенческие трейты (`artillery`/`immobile`/`hero`) остаются интерпретацией своих
  модулей — это их задокументированная модель, не долг. 13 тестов.
