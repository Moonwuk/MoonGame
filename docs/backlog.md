...
- **SHIP-7** 🔒(SHIP-1) `[proto][core]` Десантная эскадрилья = **конверсия в дивизию**
  ...
  **целевую дивизию**; стоит дороже обычной дивизии (+~25% — за доставку/конверсию);
  `cargoSize 2` (занимает 2 слота, если едет в трюме носителя). Летит сам или в носителе;
  при штурме враждебного/ничейного **захватываемого** сектора (орбита чиста — нет
  вражеского флота; Void-станция `orbit:false` — сразу) **конвертируется в выбранную
  дивизию**, та воюет на земле (reuse `runGroundCombat`/`captureGround`). **HP дивизии ∝
  доле HP эскадрильи** на момент конверсии (сбили на подлёте → слабее). Тесты.
  _(Носитель — отдельный готовый юнит `dropship`: `cargoCapacity 8` / `attack 2`.)_
- **SHIP-8** 🔒(SHIP-1) `[data][core]` Эскадрильи — малые корабли (истребитель/бомбер…),
  тип/трейт `squadron`, разные роли; едут в трюме носителя (полиморфный груз: дивизия |
  эскадрилья). Носитель и десант-юнит — тоже `squadron` (цель для ПВО, см. SHIP-10). Тесты.
