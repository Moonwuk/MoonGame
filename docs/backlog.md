...
> 853 теста зелёные. Блоки A, B, C, D закрыты целиком — ядро (этап 1) собрано.
...
- **H2** ✅ Индикация огня ПВО (+залповый режим): ПВО бьёт **раз в игровой час** по
  сетке мирового времени (перецеливание на каждом залпе; нырок в орбиту между залпами
  безнаказан). Ядро объявляет каждый залп событием `aa.fired
