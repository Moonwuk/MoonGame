...
- **EFX-1** ✅ `[core]` **Универсальный движок трейтов/эффектов.** `effectsModule`
  ...
  захватчика на узле: флот или севший десант) и `schedule` (каденс-петля по
  мирам-носителям: ленивый арм на `time.advanced`, пере-арм от `at` тика → офлайн-догон
  исполняет каждый пропущенный тик в порядке (at, seq); пол каденса 1ч fail-secure);
