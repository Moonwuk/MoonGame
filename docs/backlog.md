...
- **NETP0-1** ⏳ `[proto]` Порог победы в сети: netserver не передаёт `config` в
  `MatchRoom` → victory берёт дефолт 600, а HUD обещает 450. Передать конфиг
  (score limit + timeScale) при создании комнаты; тест.
- **NETP0-2** ⏳ `[proto]` Совет учёных для p2: `DEFAULT_SETUP` помечает p2 как AI и
...
- **NETP0-3** ⏳ `[cli]` `[proto]` Доменные события по сети: сервер уже шлёт
...
- **NETP0-4** ⏳ `[proto]` Сетевой чат: `sendChatMsg` в NET отвечает заглушкой —
  прокинуть сессионный чат через сервер (relay по образцу ally-пингов).
- **NETP0-5** ⏳ `[proto]` Переговоры между людьми: смягчение стойки (мир/пакт/союз)
  в NET заблокировано заглушкой, война необратима. Включить consent-офферы ядра
  (D2/D3) в сетевом UI — ядро уже умеет.
- **NETP0-6** ⏳ `[proto]` EN-локализация свежих окон: steward («Хранитель») и
