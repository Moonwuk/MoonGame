...
> 🔧 **В работе (devin):** открытая ветка `session-tech-tree` — `technologyModule`
> + `data/technologies.json` (тиры, пререквизиты, анлоки юнитов/зданий, эффекты
> через существующие хуки production/fleet.speed/combat.damage + гейт постройки).
> На ревью/мердж в `main` — **не брать параллельно**.
...
- **PERF-1** 🔧 **В работе (devin):** открытая ветка `engine-optimization` — горячий
  путь планировщика (`kernel.ts`: `scheduled` держится отсортированным → `earliestDue`
  O(1) + бинарная вставка) + `orbit.ts`. **Правит ядро** → ревью на сохранение
  детерминизма/семантики (golden-тест + полный прогон обязательны).
