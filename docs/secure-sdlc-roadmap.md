...
**Подзадачи:** npm provenance / **OIDC trusted publishing** (если публикуем пакеты); **SLSA**-провенанс (Build L2–L3 через hosted CI) + **cosign**/`actions/attest-build-provenance` для образов; `npm audit signatures`; OpenSSF **Scorecard**-экшен на PR. **Это бирки SEC-7 (+ часть SEC-5).** `Scorecard` уже гоняется на `main` (`security.yml`) — остальное открыто, см. `docs/backlog.md` SEC-7 (попытка `attest-build-provenance` на APK откачена: zizmor `unpinned-uses`, экшен вне scope сессии для проверки sha256).
