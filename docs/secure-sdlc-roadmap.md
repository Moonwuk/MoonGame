...
### SD-3.3 · Провенанс артефактов + проверка `[sec]` ⏳→частично ✅ (SEC-5 ✅) — M
...
**Сделано:** `actions/attest-build-provenance` на обе APK в `android.yml` — единственный
артефакт с реальной раздачей (rolling releases). `Scorecard` уже гоняется на `main`
(`security.yml`). **Не сделано:** cosign/провенанс для контейнерных образов (нет
реального деплоя, подписывать нечего — см. `security/pipeline.md`, анти-цель
security-театра); npm provenance/OIDC publishing (пакеты не публикуются); `npm audit
signatures`; SHA-пин самого `attest-build-provenance` (см. `docs/backlog.md` SEC-7).
