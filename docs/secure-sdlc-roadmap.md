...
- **SEC-0 ✅** — пайплайн (GitHub Actions, `.github/workflows/security.yml`); после SEC-1
  ...
  информационные (`continue-on-error`): `pnpm run check`
  (lint+typecheck+test+docs-check), Semgrep + CodeQL SAST, Gitleaks + TruffleHog (секреты),
...
✅ уже в коде: Dependabot настроен — `.github/dependabot.yml` (**четыре экосистемы**: npm `/`, npm `/mobile`, github-actions, docker; еженедельно, dev-deps сгруппированы, **cooldown `default-days: 7`** на всех); osv-scanner уже покрывает `mobile/package-lock.json`; экшены пиннятся по SHA. Остаётся: авто-мердж patch/minor после выдержки, PR-режим «только новые vulns», ежеквартальная прополка.
