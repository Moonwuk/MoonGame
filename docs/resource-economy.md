...
### ECON-3 · Ножницы: metal-сток и credits-кран `[proto][core]` — M · ✅ (2026-07-22)

**Как легло:** `econScrewsModule` (game.ts) + обе zod-схемы в SV-1.2. Отклонения от
плана: (а) дополнительно `E_IN_BATTLE` (док в осаде не чинит — паритет с платным
мгновенным ремонтом, который появился раньше кирпича и делит с ним `missingHull`);
(б) отказ без плавильни — код `E_NO_SMELTER`, вместо степпера — пресеты
`20⬢/100⬢/всё`; кнопка ремонта — металл-чип «🔧 N⬢» в ХП-ряду карточки (рядом с
