...
> **Гейт:** `pnpm run check` (lint + typecheck + test). **Тесты: 1362 зелёных** (28 skip, 130 файлов).
...
брендново-новому нику, переживает reload.
**✅ ONB-1 (spotlight-движок гайд-марок):** чистый движок `prototype/src/spotlight.ts`
(шаги из данных, `parseAdvance`/`parseTour` fail-secure, продвижение по
`tap`/`action:<type>`/`state:<pred>`, пропуск, optional-скип/безопасный стоп; иммутабельно,
как `onboarding.ts`) + тонкий DOM-контроллер `spotlightDom.ts` (оверлей+подсветка bbox+пузырь
+счётчик «Шаг k из n»+«Пропустить»/«Далее», `poll()` re-query цели — устойчив к перерисовке,
`notifyAction` из игрового цикла, `onStep/onComplete/onSkip`; **инжектируемый `document`** →
тест без браузера) + CSS в `build.mjs` (слой 44, click-through) + i18n RU/EN. Гайдовый матч
(конкретный тур + хуки в `frame()`) — ONB-2 (`docs/onboarding-roadmap.md`).
