...
**AVA-7 оркестратор (S4):** `AvaOrchestrator.launchDue` — LOCKED-матчап без `matchId`
→ `pickAvaMap` (seed = id матчапа, воспроизводимо; карта под бо́льшую сторону) →
раскладка ростера по командным слотам (союзники сгруппированы раскладкой карты;
недобранная сторона добита ИИ-местами `ai-<slot>` — офлайн-оборона без
авто-наступления) → `buildStateFromMap({slots, crossTeamStart:'peace'})` (S5: бой
заперт до AVA-8) → seq-0 снапшот в `MatchStore` — `LazyRoomRegistry` поднимает
сессию на первом коннекте, транспорт/фог/персистентность переиспользованы. Личность
места: `playerId = accountId` (ростер = карта мест, новая колонка только
`matchup.match_id`). Launch exactly-once (`bindMatch` — условный UPDATE; порядок
save→bind + детерминированный `ava-<matchupId>` обезвреживают гонку).
`resolveAvaSeat` в join-пути `main.ts` ПЕРЕД `resolveSeat`: ростерный → фикс-место,
чужой/аноним → `E_NOT_ROSTERED` 403, обычный матч → штатный резолвер. Запуск —
хвостом ростер-свипа. Дальше: мир→война→итог (AVA-8) + лента (AVA-9),
`corporation-wars.md`.
