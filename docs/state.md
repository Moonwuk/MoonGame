...
- `diplomacy?: Record<pairKey, DiplomaticStance>` — попарные дип-отношения (`war`/`peace`/
  ...
  `state.diplomacy`** (`getStance(...) === 'war'`) — бой идёт только при объявленной войне. **ПВО бьёт залпами раз в игровой час** (не непрерывно):
  сетка залпов — границы игрового часа мирового времени (×timeScale, `roundIntervalMs`),
  один полный залп (`aaDamage`-сумма) за каждую пересечённую границу, перецеливание на
  каждом залпе; флот, нырнувший в орбиту МЕЖДУ залпами, уходит невредимым — тайминг
  рейда мимо ПВО имеет смысл. Каждый залп — событие `aa.fired {planetId, owner, fleetId,
