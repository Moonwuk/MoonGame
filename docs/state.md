...
- `diplomacy?: Record<pairKey, DiplomaticStance>` — попарные дип-отношения (`war`/`peace`/
  ...
  **Сессионное меню дипломатии/сообщений** (прототип, рейл → Дипломатия/Dispatches):
  ...
  на симуляцию не влияют). На сервере уже есть эфемерная ally-ping система (`protocol.ts`
  `Ping`/`ping.place`/`ping.added`, видимость по командам, TTL) — клиент к ней подключается
  отдельным шагом.
