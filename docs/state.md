...
> **Гейт:** `pnpm run check` (lint + typecheck + test). **Тесты: 903 зелёных** (4 skip, 90 файлов).
...
- `diplomacy?: Record<pairKey, DiplomaticStance>` — попарные дип-отношения (`war`/`peace`/
  ...
  **Сеть честна (№10)**: в NET-матче композер чата задизейблен с подписью «сетевой чат
  ещё не подключён» (сообщения были плацебо — писались только в локальный массив), пинги 📍
  остаются рабочим сетевым каналом; смены стоек между снапшотами диффаются клиентом —
  «⚔ X объявил вам войну!» всплывает тостом + бейдж непрочитанного на ✉ рейла.
  ...
  **Сессионное меню дипломатии/сообщений** (прототип, рейл → Дипломатия/Dispatches):
  ...
  смена стойки предложениями — повышение до мира/пакта требует согласия ИИ
  (детерминированно по числу провинций), **союз с ИИ невозможен** (кнопка погашена,
...
- **Цепочки приказов (command-chains).** Простаивающий флот сам проходит цепочку
  ...
  **Комфорт-батч (аудит «удобные цепочки»):** провал шага **не** скипается молча —
  ...
  лимиты CC-5.1 (`MAX_CHAIN_STEPS`=32, `wait`≤720ч, `E_QUEUE_FULL`); правка по шагу
  `order.remove {index}` (✕ в панели, «↩ последний»); **🔁 повтор** — маркер-шаг
  `repeat`, `popChainStep` ротирует исполненный шаг в хвост (патруль оффлайн); шаги
  `unload`/`bombard` (`unloadHereActions`); живой приказ (Move/Stop/штурм/обстрел/
  отступление/слияние) **снимает план** (`dropChains`, тост); режим «➕ строить» без
  ловушек (сброс в `clearSelection`/Back, канвас-баннер, тосты «шаг N: → мир · весь
  план ≈Xч», hit-тест флотов/пингов пропускается); план **виден на карте**
  (`drawChainPlans`: тусклый пунктир по `planRoute` + нумерованные пипы + глифы
  ⚔/▲/▼/☄/⏸/🔁, бейдж `▸N`/`⚠` на маркере флота); превью+ETA при постройке
  (`drawAimPreview` от `chainTailNode`, `chainEta`/`fmtHrs`, накопительное ETA шагов
  и итог плана в панели); маршрут через мир при мире — бейдж «⚔ нужна война» в момент
  постройки; `state.orders` **фильтруется в fog** (`visibility.project`: только свои
  флоты — план не утекает врагу); в NET `qauto`/`qscramble` честно задизейблены
  («пока только в одиночной»); группа флотов строит план теми же кнопками.
