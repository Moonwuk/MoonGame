...
> **Гейт:** `pnpm run check` (lint + typecheck + test). **Тесты: 587 зелёных** (4 skip, 65 файлов).
...
- `packages/client` — клиент (Этап 4): направление **PWA-first веб-клиент** (TWA Android + Capacitor iOS, не React Native — см. `cross-platform-roadmap.md`). Есть минимальный `MultiplayerClient` transport adapter; app shell ещё плейсхолдер.
...
- **Лечение/ремонт на `time.advanced`:** (а) **гарнизон** лечится по сумме
  `healRate` зданий (госпиталь). (б) **Корабли флота** (`fleet.units`) — урон корпуса
  теперь постоянный (нет авто-лечения после боя), и чинится здесь: **щиты** (полоса HP
  **выше 30%**) восстанавливаются между боями где угодно (`SHIELD_REGEN`/ч); **корпус**
  (полоса **≤30%** — пробоина) чинится только пока флот стоит над **дружественным** миром
  (базовый темп + бонус `healRate` госпиталя/верфи), и до ремонта **тянет скорость вниз**
  (`route.ts fleetBaseSpeed` — штраф <30%). Флот в бою не регенит.
...
```bash
pnpm install
pnpm run check       # lint + typecheck + test (гейт; 587 тестов)
pnpm test            # vitest
