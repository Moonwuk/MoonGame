...
> **Гейт:** `pnpm run check` (lint + typecheck + test). **Тесты: 122 зелёных.**
...
packages/shared-core/src/
  ...
  action/        types.ts (Action, Context, MatchConfig.timeScale, ApplyResult/AdvanceResult, Rejection, timeScaleOf)
  data/          schemas.ts (zod-схемы + parseGameData, buildingLevel/buildingMaxLevel)
  rng/           rng.ts (sfc32)
  util/          clone.ts (deepClone/deepFreeze)
  modules/       economy, movement, sector, combat, construction, army  (+ *.test.ts рядом)
...
Порядок в кернелах обычно: `sector, economy, movement, combat, construction, army`.
...
- **Реальное ядро** в браузере: `createKernel([sector, economy, movement, combat,
construction, army, fleetLaunch])`, тик в реальном времени (скорость ⏸/▶/⏩).
...
(лейны + Дейкстра) и движение; **типы секторов**; бой (раунды, линии,
attack/defense); **двухфазный захват орбита→десант**; **орбиты near/far + ручной
штурм + бомбардировка (заморозка производства) + орбитальное ПВО**; **здания**
(инстансы, уровни/апгрейд, HP, бонус обороны, разрушение); **разделение флота и
наземной армии + транспорт (load/unload)**; играбельный прототип с тактическим
векторно-радарным UI (DEFCON-вайб).

**⏳ Дальше (по плану):**

1. **Типы планет** (как сектора, через хуки).
2. **Фракции** — стартовые лоадауты + уникальные юниты/механики.
3. **Сессионное древо технологий** — разблок/бонусы/старт, фиксируется на матч.
4. **UI-стадия** — реальные орбитальные контролы (near/far, bombard), погрузка
   армии, конструктор наземных юнитов, панели, предматчевый экран.
5. Модуль **очков/победы** (§8), движок **трейтов**, **видимость/туман войны**,
   дипломатия/пинги/шпионаж (внутрисессионные модули).
6. **Этап 2** (слой действий: валидация/авторизация/идемпотентность,
   персистентность), **Этап 3** (сервер + мета-галактика), **Этап 4** (RN-клиент).
...
```bash
pnpm install
pnpm run check       # lint + typecheck + test (гейт; 122 теста)
pnpm test            # vitest
