...
> **Гейт:** `pnpm run check` (lint + typecheck + test). **Тесты: 1027 зелёных** (4 skip, 104 файла).
...
MatchKeeper, аккаунты логин/пароль + JWT (opt-in). Дальше: включить auth/gate на
играбельном пути (прототип-хост), OIDC, мультипроцесс.
