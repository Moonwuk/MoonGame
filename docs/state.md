...
- `diplomacy?: Record<pairKey, DiplomaticStance>` — попарные дип-отношения (`war`/`peace`/
  ...
  **Сессионное меню дипломатии/сообщений** (прототип, рейл → Дипломатия/Dispatches):
  ...
  согласия ИИ (детерминированно по числу провинций), понижение/война односторонни; лента
  сообщений = свободный текст игрока + системные дип-события (через `diplomacy.changed`).
  Сообщения живут в клиенте (не в ядре — на симуляцию не влияют).
